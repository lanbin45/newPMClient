...
import {Header,Footer,HeaderColumnGroup,FooterColumnGroup,PrimeTemplate} from 'primeng/primeng';
...
@Component({
    selector: 'p-rowExpansionLoader1',
    template: ``
})
export class RowExpansionLoader1 implements OnInit, OnDestroy {
...
export class Column1 implements AfterContentInit{
    @Input() field: string;
    @Input() colId: string;
    ...
    template: `
        <ng-template ngFor let-col [ngForOf]="columns" let-lastCol="last">
            <th #headerCell [attr.id]="col.colId" [ngStyle]="col.headerStyle||col.style" [class]="col.headerStyleClass||col.styleClass" (click)="dt.sort($event,col)" [attr.colspan]="col.colspan" [attr.rowspan]="col.rowspan"
                ...
                <span class="ui-column-resizer ui-clickable" *ngIf="dt.resizableColumns && ((dt.columnResizeMode == 'fit' && !lastCol) || dt.columnResizeMode == 'expand')" (mousedown)="dt.initColumnResize($event,frozenOther)"></span>
                ...
                <p-dtCheckbox1 *ngIf="col.selectionMode=='multiple' && dt.showHeaderCheckbox === true" (onChange)="dt.toggleRowsWithCheckbox($event)" [checked]="dt.allSelected" [disabled]="dt.isEmpty()"></p-dtCheckbox1>
            </th>
        </ng-template>
    `
})
export class ColumnHeaders1 {

    constructor(@Inject(forwardRef(() => DataTableList)) public dt:DataTableList) {}

    @Input("pColumnHeaders1") columns: Column1[];
    @Input("frozenOther") frozenOther: boolean;
}

@Component({
    selector: '[pColumnFooters1]',
    template: `
...
export class ColumnFooters1 {

    constructor(@Inject(forwardRef(() => DataTableList)) public dt:DataTableList) {}

    @Input("pColumnFooters1") columns: Column1[];
}

@Component({
    selector: '[pTableBody1]',
    template: `
            ...
            <tr #rowElement *ngIf="!dt.expandableRowGroups||dt.isRowGroupExpanded(rowData)"
                ...
                <ng-template ngFor let-col [ngForOf]="columns" let-colIndex="index">
                    <td #cell *ngIf="!dt.rowGroupMode || (dt.rowGroupMode == 'subheader') ||
                        ...
                        <p-dtRadioButton1 *ngIf="col.selectionMode=='single'" (onClick)="dt.selectRowWithRadio($event, rowData)" [checked]="dt.isSelected(rowData)"></p-dtRadioButton1>
                        <p-dtCheckbox1 *ngIf="col.selectionMode=='multiple'" (onChange)="dt.toggleRowWithCheckbox($event,rowData)" [checked]="dt.isSelected(rowData)"></p-dtCheckbox1>
                    </td>
                </ng-template>
            </tr>
            <tr *ngIf="dt.expandableRows && dt.isRowExpanded(rowData)" class="ui-expanded-row-content">
                <td [attr.colspan]="dt.visibleColumns().length">
                    <p-rowExpansionLoader1 [rowData]="rowData" [rowIndex]="rowIndex" [template]="dt.rowExpansionTemplate"></p-rowExpansionLoader1>
                </td>
            </tr>
...
export class TableBody1 {

    constructor(@Inject(forwardRef(() => DataTableList)) public dt:DataTableList) {}

    @Input("pTableBody1") columns: Column1[];
            ...
            <div #scrollHeaderBox  class="ui-datatable-scrollable-header-box">
                <table [ngClass]="dt.tableStyleClass" [ngStyle]="dt.tableStyle">
                    <thead class="ui-datatable-thead">
                        <tr *ngIf="!headerColumnGroup" class="ui-state-default" [pColumnHeaders1]="columns" [frozenOther]="frozenOther"></tr>
                        <ng-template [ngIf]="headerColumnGroup">
                            <tr *ngFor="let headerRow of headerColumnGroup.rows" class="ui-state-default" [pColumnHeaders1]="headerRow.columns" ></tr>
                        </ng-template>
                    </thead>
                    <tbody *ngIf="dt.frozenValue" [ngClass]="{'ui-datatable-data ui-widget-content': true, 'ui-datatable-hoverable-rows': (dt.rowHover||dt.selectionMode)}" [pTableBody1]="columns" [data]="dt.frozenValue"></tbody>
                </table>
            </div>
        </div>
        <div #scrollBody class="ui-datatable-scrollable-body" [ngStyle]="{'width': width}">
            <div #scrollTableWrapper class="ui-datatable-scrollable-table-wrapper" style="position:relative">
                <table #scrollTable [class]="dt.tableStyleClass" [ngStyle]="dt.tableStyle" [ngClass]="{'ui-datatable-virtual-table':virtualScroll}" style="top:0px">
                    ...
                    <tbody [ngClass]="{'ui-datatable-data ui-widget-content': true, 'ui-datatable-hoverable-rows': (dt.rowHover||dt.selectionMode)}" [pTableBody1]="columns" [data]="dt.dataToRender"></tbody>
                </table>
            </div>
        </div>
            ...
            <div #scrollFooterBox  class="ui-datatable-scrollable-footer-box">
                <table [ngClass]="dt.tableStyleClass" [ngStyle]="dt.tableStyle">
                    <tfoot class="ui-datatable-tfoot">
                        <tr *ngIf="!footerColumnGroup" [pColumnFooters1]="columns" class="ui-state-default"></tr>
                        <ng-template [ngIf]="footerColumnGroup">
                            <tr *ngFor="let footerRow of footerColumnGroup.rows" class="ui-state-default" [pColumnFooters1]="footerRow.columns"></tr>
                        </ng-template>
                    </tfoot>
                </table>
            </div>
...
export class ScrollableView1 implements AfterViewInit,AfterViewChecked,OnDestroy {

    constructor(@Inject(forwardRef(() => DataTableList)) public dt:DataTableList, public domHandler: DomHandler, public el: ElementRef, public renderer: Renderer2, public zone: NgZone) {}

    @Input("pScrollableView1") columns: Column1[];
...
@Component({
        ...
        <div [ngStyle]="style" [class]="styleClass" [style.width]="containerWidth"
            ...
            <div class="ui-datatable-tablewrapper" *ngIf="!scrollable">
                <table [ngClass]="tableStyleClass" [ngStyle]="tableStyle">
                    <thead class="ui-datatable-thead">
                        <tr *ngIf="!headerColumnGroups.first" class="ui-state-default" [pColumnHeaders1]="columns"></tr>
                        <ng-template [ngIf]="headerColumnGroups.first">
                            <tr *ngFor="let headerRow of headerColumnGroups.first.rows" class="ui-state-default" [pColumnHeaders1]="headerRow.columns"></tr>
                        </ng-template>
                    </thead>
                    <tfoot *ngIf="hasFooter()" class="ui-datatable-tfoot">
                        <tr *ngIf="!footerColumnGroups.first" class="ui-state-default" [pColumnFooters1]="columns"></tr>
                        <ng-template [ngIf]="footerColumnGroups.first">
                            <tr *ngFor="let footerRow of footerColumnGroups.first.rows" class="ui-state-default" [pColumnFooters1]="footerRow.columns"></tr>
                        </ng-template>
                    </tfoot>
                    <tbody [ngClass]="{'ui-datatable-data ui-widget-content': true, 'ui-datatable-hoverable-rows': (rowHover||selectionMode)}" [pTableBody1]="columns" [data]="dataToRender"></tbody>
                </table>
            </div>
            
            <ng-template [ngIf]="scrollable">
                <div class="ui-datatable-scrollable-wrapper ui-helper-clearfix">
                    <div *ngIf="hasFrozenColumns()" [pScrollableView1]="frozenColumns" frozen="true"
                    ...
                    <div [pScrollableView1]="scrollableColumns" [frozenOther]="hasFrozenColumns()" [ngStyle]="{'width':this.unfrozenWidth, 'left': this.frozenWidth}"
    ...
    `,
    providers: [DomHandler,ObjectUtils1]
})
export class DataTableList implements AfterViewChecked,AfterViewInit,AfterContentInit,OnInit,OnDestroy,BlockableUI {
    ...
    @ContentChildren(Column1) cols: QueryList<Column1>;
    ...
    public columns: Column1[];

    public frozenColumns: Column1[];

    public scrollableColumns: Column1[];
    ...
    constructor(public el: ElementRef, public domHandler: DomHandler, public differs: IterableDiffers,
                public renderer: Renderer2, public changeDetector: ChangeDetectorRef, public objectUtils: ObjectUtils1,
                public zone: NgZone) {
    ...
    onHeaderKeydown(event, column: Column1) {
        if(event.keyCode == 13) {
    ...
    switchCellToEditMode(cell: any, column: Column1, rowData: any) {
    ...
    onCellEditorKeydown(event, column: Column1, rowData: any, rowIndex: number) {
        if(this.editable) {
            //enter
            if(event.keyCode == 13) {
    ...
    onCellEditorInput(event, column: Column1, rowData: any, rowIndex: number) {
        if(this.editable) {
            this.onEdit.emit({originalEvent: event, column: column, data: rowData, index: rowIndex});
        }
    }

    onCellEditorChange(event, column: Column1, rowData: any, rowIndex: number) {
        if(this.editable) {
    ...
    onCellEditorBlur(event, column: Column1, rowData: any, rowIndex: number) {
        if(this.editable) {
            if(this.editChanged)
    ...
    onColumnResizeEnd(event) {
        ...
        if(columnWidth + delta > parseInt(minWidth)) {
            if(this.columnResizeMode === 'fit') {
                ...
                let newNextColumnWidth = nextColumn.offsetWidth;
                ...
                if(newColumnWidth > 15 && nextColumnWidth > 15) {
                    ...
                    /*if(delta>0) {
                        for (var i = 0; i < delta; i++) {
                            ...
                            columnWidth++;
                            this.resizeColumn.style.width = columnWidth + 'px';
                    ...
                    if(this.scrollable) {
                        ...
                        let colGroup;
                        if(this.resizeColumn.frozenOther){
                            let c = this.domHandler.find(this.el.nativeElement, 'colgroup.ui-datatable-scrollable-colgroup');
                            if(c.length > 1) {
                                colGroup = c[1];
                            }
                        }else{
                            colGroup = this.domHandler.findSingle(this.el.nativeElement, 'colgroup.ui-datatable-scrollable-colgroup');
                        }
    ...
    fixColumnWidths() {
        ...
        if(this.scrollable) {
            ...
            if(bodyCols) {
                ...
                for(let i = 0; i < columns.length; i++) {
                    if (bodyCols.length <= i && cols2) {
                        cols2[i-bodyCols.length].style.width = (columns[i].offsetWidth) + 'px';
                    } else {
...
@NgModule({
    ...
    exports: [DataTableList,SharedModule,Column1],
    declarations: [DataTableList,Column1,DTRadioButton1,DTCheckbox1,ColumnHeaders1,ColumnFooters1,TableBody1,ScrollableView1,RowExpansionLoader1]
})
export class DataTableListModule { }
