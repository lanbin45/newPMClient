...
import {DomHandler} from '../../../../../node_modules/_primeng@4.3.0@primeng/components/dom/domhandler'

@Directive({
    selector: '[ppTooltip]',
    providers: [DomHandler]
})
export class ppTooltip implements OnDestroy {
    ...
    @Input('ppTooltip') set text(text: string) {
        this._text = text;
        if(this.active) {
            if(this._text) {
...
@NgModule({
    ...
    exports: [ppTooltip],
    declarations: [ppTooltip]
})
export class ppTooltipModule { }
