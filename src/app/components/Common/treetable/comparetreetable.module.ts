...
@Component({
    selector: '[gridRow]',
    styleUrls :['comparetreetable.css'],
    template: `
        <div [class]="node.styleClass" [ngClass]="{'ui-treetable-row': true, 'ui-even-highlight':isEven(),'ui-diff-even-highlight':isEvenDiff(),'ui-diff-odd-highlight':isOddDiff(),
        'ui-state-highlight':isSelected(),'ui-treetable-row-selectable':treeTable.selectionMode && node.selectable !== false}">
            <td *ngFor="let col of treeTable.columns; let i=index" [ngStyle]="col.style" [class]="col.styleClass" (click)="onRowClick($event)" (dblclick)="rowDblClick($event)" (touchend)="onRowTouchEnd()" (contextmenu)="onRowRightClick($event)">
                ...
                <p-columnBodyTemplateLoader [column]="col" [rowData]="node" [rowIndex]="rIndex" *ngIf="col.template"></p-columnBodyTemplateLoader>
            </td>
        </div>
        <div *ngIf="node.children && node.expanded" class="ui-treetable-row" style="display:table-row">
            <td [attr.colspan]="treeTable.columns.length" class="ui-treetable-child-table-container">
                <table [class]="treeTable.tableStyleClass" [ngStyle]="treeTable.tableStyle">
                    <tbody gridRow *ngFor="let childNode of node.children;index as rowIndex" [showdiff]="showdiff" [rIndex]="rIndex+rowIndex+1" [node]="childNode" [level]="level+1" [labelExpand]="labelExpand" [labelCollapse]="labelCollapse" [parentNode]="node"></tbody>
                </table>
            </td>
        </div>
    `
...
export class GridRow  implements OnInit {
    ...
    constructor(@Inject(forwardRef(() => CompareTreeTable)) public treeTable:CompareTreeTable) {}
    ...
    isEvenDiff(){
        if(this.node.data) {
            if(this.node.data.targetvalue !== this.node.data.mastervalue && this.showdiff){
                if(this.rIndex%2==0){
                    return true;
                }else{
                    return false;
        ...
        <div [ngClass]="'ui-treetable ui-widget'" [ngStyle]="style" [class]="styleClass">
            ...
            <div class="ui-treetable-tablewrapper">
                <table #tbl class="ui-widget-content" [class]="tableStyleClass" [ngStyle]="tableStyle">
                    ...
                    <tbody gridRow *ngFor="let node of value;index as rowIndex;" [showdiff]="showdiff" [rIndex]="rowIndex" class="ui-treetable-data ui-widget-content" [node]="node" [level]="0" [labelExpand]="labelExpand" [labelCollapse]="labelCollapse"></tbody>
                </table>
            </div>
...
export class CompareTreeTable implements AfterContentInit {
...
@NgModule({
    ...
    exports: [CompareTreeTable],
    declarations: [CompareTreeTable,GridRow]
})
export class CompareTreeTableModule { }